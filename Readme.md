...
As I mentioned above, we will manually set up database through this tutorial. (This will help you understand how it's working.)

In order to connect to SQL Edge from SQL client, open inbound 1433 port in firewall settings on Edge device (Ubuntu) networking settings.

![Open default SQL port](images/open_sql_port.png?raw=true)

Connect Azure SQL Edge database using SQL client in your working machine, such as, Azure Data Studio.<br>

- Server Name : [IP address of your Edge device (Ubuntu)]
- Admin User Name : sa
- Admin User Password : P@ssw0rd

Run ```sql/create_schema.sql``` on SQL Edge database.
...
By running this script, the streaming events generated by Data Generator Module will be consumed by streaming job on Azure SQL Edge, and the all events will be saved into ```InspectionResults``` table.
...
In this exercise, ```grade``` column in ```InspectionResults``` table will all be empty.

![Open default SQL port](images/table_data01.png?raw=true)

## Deploy Model on SQL Edge ##

In this exercise, we will create the model to predict ```grade``` column of ```InspectionResults``` table in Azure Machine Learning notebook.<br>
The model will be deployed into SQL Edge database, and the value of ```grade``` column will be predicted (inferenced) row by row within database process.
...
Go to [machine learning studio](https://ml.azure.com/) and login to above workspace.<br>
Click "Compute" in left-navigation and create a compute instance in machine learning studio.

![Create machine learning compute instance](images/create_ml_compute.png?raw=true)

Click "Notebooks" in the left navigation.<br>
Push "Upload files" icon button and upload ```train-model/train.ipynb``` and ```train-model/traindat.csv``` in this repository.

![Upload files on ML compute instance](images/upload_files_on_mlcompute.png?raw=true)

Change the following settings in ```train.ipynb```.
...
This will create ONNX model file for predicting the value of ```grade``` column, and the generated model (binary) will be inserted into ```Models``` table.

Finally, run ```sql/register_prediction_agent.sql``` on SQL Edge database.<br>
...
- Set values of ```grade``` column in ```InspectionResults``` table using our trained model.
